/*
 * Bangumi API
 *
 * 你可以在 <https://next.bgm.tv/demo/access-token> 生成一个 Access Token  ## [关于 User Agent](https://github.com/bangumi/api/blob/master/docs-raw/user%20agent.md)  如果你在使用中遇到了问题，请优先使用 GitHub issue 提交问题。在 bangumi 小组发帖可能无法得到及时反馈。 
 *
 * The version of the OpenAPI document: 2025-07-1
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct PersonRevisionProfession {
    #[serde(rename = "producer", skip_serializing_if = "Option::is_none")]
    pub producer: Option<String>,
    #[serde(rename = "mangaka", skip_serializing_if = "Option::is_none")]
    pub mangaka: Option<String>,
    #[serde(rename = "artist", skip_serializing_if = "Option::is_none")]
    pub artist: Option<String>,
    #[serde(rename = "seiyu", skip_serializing_if = "Option::is_none")]
    pub seiyu: Option<String>,
    #[serde(rename = "writer", skip_serializing_if = "Option::is_none")]
    pub writer: Option<String>,
    #[serde(rename = "illustrator", skip_serializing_if = "Option::is_none")]
    pub illustrator: Option<String>,
    #[serde(rename = "actor", skip_serializing_if = "Option::is_none")]
    pub actor: Option<String>,
}

impl PersonRevisionProfession {
    pub fn new() -> PersonRevisionProfession {
        PersonRevisionProfession {
            producer: None,
            mangaka: None,
            artist: None,
            seiyu: None,
            writer: None,
            illustrator: None,
            actor: None,
        }
    }
}

